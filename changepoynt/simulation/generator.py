...
class ChangeSignalGenerator:
    ...
    def __init__(self,
                 ...
                 random_generator: np.ndarray = None,
    ...
    def generate_random_points(self, maximum_value: int, minimum_distance: int, number_of_points: typing.Optional[int] = None) -> list[int,]:

        # check whether we need a default value of points
        if number_of_points is None:
            number_of_points = self.random_generator.integers(0, self.max_points(maximum_value, minimum_distance), endpoint=True)
        ...
        if available_space < 0:
            raise ValueError(f"Not enough space for the given '{maximum_value=}', '{minimum_distance=}', and 'number_of_points={number_of_points-2}'. We can only have a maximum of: {self.max_points(maximum_value, minimum_distance)}).")
    ...
    def generate_dependent_list(self, original_lists: list[list[int]], linking_weights: list[float,]):
        ...
        assert len(linking_weights) == signal_number, f"We require a linking value for each original_list. Currently. '{len(linking_weights)=}' not equal to '{signal_number=}'."

        # check that the weights sum up to one
        assert abs(sum(linking_weights) - 1.0) < 1e-6, f"Linking weights must sum to 1. Current sum: {sum(linking_weights)}."
    ...
    def generate_number_of_events(self):
        ...
        if sampled_event_number > max_events:
            warnings.warn(f'The sampled event number ({sampled_event_number}) if larger than the allowed events. Defaulting to {max_events=}.')
    ...
    def generate_independent_lists(self, num_lists: int):
        ...
        if num_events < num_lists:
            raise ValueError(f"We could not sample enough change points for the specified number of lists '{num_lists=}'. Either decrease 'num_lists', decrease 'samples_before_change', or increase 'rate'.")
    ...
    def generate_independent_list_disturbed(self, num_lists: int, extra_lists: typing.Optional[int] = None):
        ...
        try:
            ...
            raise ValueError(f"We tried to generate {num_lists + extra_lists=} independent lists. Most likely these were to much. Original error below.\n\n{excp}.")
...
if __name__ == '__main__':
    cpg = ChangeSignalGenerator(length=10000)
