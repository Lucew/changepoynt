...
def test_msst_unknown_method_raises_value_error():
...
def test_msst_rejects_1d_input():
    signal, _ = _make_change_signal()
    detector = MSST(window_length=40, method="rsvd")
    with pytest.raises(AssertionError):
        detector.transform(signal)


def test_msst_rejects_too_short_signal():
    detector = MSST(window_length=40, n_windows=20, lag=20, method="rsvd")
    too_short = np.linspace(0.0, 1.0, 80)
    with pytest.raises(AssertionError):
        detector.transform(too_short)


def test_msst_score_is_zero_before_first_possible_output():
    signal, _ = _make_change_signal()
    detector = MSST(window_length=40, n_windows=20, lag=20, method="rsvd")
    np.random.seed(7)
...
def test_msst_detects_frequency_change_near_boundary():
    signal, change_idx = _make_change_signal()
    detector = MSST(window_length=48, n_windows=24, lag=24, rank=2, method="rsvd")
    ...
    assert neighborhood.max() > np.percentile(outside, 98)


def test_msst_fast_hankel_tracks_reference_implementation():
    signal, _ = _make_change_signal()
...
def test_msst_tracks_esst():
    signal, _ = _make_change_signal()
    slow = MSST(window_length=40, n_windows=20, lag=20, rank=2, method="rsvd", use_fast_hankel=False)
    fast = MSST(window_length=40, n_windows=20, lag=20, rank=2, method="rsvd", use_fast_hankel=True)
    orig_esst = cpsst.SST(window_length=40, n_windows=20, lag=20, rank=2, method="rsvd", use_fast_hankel=False)
